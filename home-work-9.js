...
const getUsersWithFriend = (users, friendName) => {
  const usersWidthFriend = users.filter(function (user) {
    return user.friends.includes(friendName);
  })
  return usersWidthFriend;
};
...
const getSortedUniqueSkills = users => {
  return users.reduce((acc, { skills }) => [...acc, ...skills],[]).filter((skill, index, arr) => arr.indexOf(skill) === index).sort()
};
